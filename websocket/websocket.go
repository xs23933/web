...
// New new websocket
func New(handler func(*Conn), config ...Config) func(*web.Ctx) {
	var cfg Config
	if len(config) > 0 {
		cfg = config[0]
	}
	...
	var upgrader = websocket.FastHTTPUpgrader{
		...
		CheckOrigin: func(fctx *fasthttp.RequestCtx) bool {
			...
			origin := web.GetString(fctx.Request.Header.Peek("Origin"))
	...
	return func(c *web.Ctx) {
		...
		c.RequestCtx.VisitUserValues(func(key []byte, value interface{}) {
			conn.vars[string(key)] = value
		})
		// params
		params := c.Router().Params
		for i := 0; i < len(params); i++ {
			conn.params[web.ImmutableString(params[i])] = web.ImmutableString(c.Params(params[i]))
		}
		// queries
...
func acquireConn() *Conn {
	...
	conn.ID = uid.New()
	conn.vars = make(map[string]interface{})
...
// Vars Vars
func (conn *Conn) Vars(key string) interface{} {
	return conn.vars[key]
}
...
// WebSocket protocol.
func IsWebSocketUpgrade(ctx *web.Ctx) bool {
	return websocket.FastHTTPIsWebSocketUpgrade(ctx.RequestCtx)
}
